...
export default function GamePage() {
  ...
  const [currentPlayer, setCurrentPlayer] = useState(location.state?.currentPlayer || null);
  const [substring, setSubstring] = useState(location.state?.substring || '');
  const [timerDuration, setTimerDuration] = useState(location.state?.timerDuration || 15);
  ...
  const timerColor = timerPercent > 50 ? '#22d3a0' : timerPercent > 25 ? '#ff8c42' : '#ff4d6d';

  if (gameOver) {
    return <GameOverScreen winner={winner} players={players} myId={myId} onHome={() => navigate('/')} />;
  }

  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', maxWidth: '800px', margin: '0 auto', padding: '16px' }}>
      {/* Top Ad */}
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{
            ...
            {players.map((player, i) => (
              <PlayerChip
                key={player.id || i}
                player={player}
                isActive={currentPlayer?.id === player.id || currentPlayer?.socketId === player.socketId}
          ...
          {lastAction && (
            <div style={{
              ...
              background: lastAction.type === 'correct' ? 'rgba(34,211,160,0.1)' : 'rgba(255,77,109,0.08)',
              border: `1px solid ${lastAction.type === 'correct' ? 'rgba(34,211,160,0.3)' : 'rgba(255,77,109,0.2)'}`,
              borderRadius: '10px', marginBottom: '16px',
              fontSize: '13px',
              color: lastAction.type === 'correct' ? 'var(--green)' : '#ff8c42',
              animation: 'fadeIn 0.3s ease',
            }}>
              {lastAction.type === 'correct' ? '✓ ' : '⚡ '}{lastAction.msg}
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: '11px', display: 'flex', gap: '1px' }}>
        {player.isAlive
          ? Array.from({ length: player.lives }).map((_, i) => <span key={i}>❤️</span>)
