...
export default function BluffGamePage() {
  ...
  useEffect(() => {
    ...
    const onState = (state) => {
      ...
      if (prevStatus.current === 'waiting' && state.status === 'playing') {
        setShuffleAnim(true);
        setTimeout(() => { setShuffleAnim(false); setDealAnim(true); }, 900);
    ...
    const onPassed     = ({ playerName }) => {
      if (playerName !== getMyPlayerStatic()?.name) toast(`${playerName} passed`, { icon: '⏭️' });
    };
    const onGameOver   = ({ winner }) => logger.info('[Bluff] Game over', { winner: winner?.name });
    const onPlayerJoined = ({ playerName }) => toast(`${playerName} joined!`, { icon: '👋' });
    const onPlayerLeft   = ({ playerName }) => toast(`${playerName} left`, { icon: '🚪' });
    const onError        = ({ message })    => { toast.error(message); logger.error('[Bluff] Error', { message }); };
  ...
  const handleChallenge = () => socket.emit('bluff_challenge', { roomCode });
  const handlePass      = () => socket.emit('bluff_pass', { roomCode });
...
function PileDisplay({ gameState, shuffleAnim, dealAnim, playedAnim }) {
  ...
  return (
    <div style={{
      ...
      {/* Card stack visual */}
      <div style={{ position: 'relative', height: '80px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '8px' }}>
        {shuffleAnim ? (
          ...
          <div style={{ position: 'relative', width: '120px', height: '72px' }}>
            {['-12deg', '0deg', '12deg'].map((rot, i) => (
              <div key={i} style={{
                position: 'absolute', left: `${i * 10}px`, top: 0,
                width: '48px', height: '68px',
                background: 'linear-gradient(135deg, #4c1d95, #7c3aed)',
                borderRadius: '8px', border: '1.5px solid rgba(167,139,250,0.4)',
          ...
          <div style={{ position: 'relative', width: `${Math.min(gameState.pileCount * 3 + 48, 90)}px`, height: '72px' }}>
            {Array.from({ length: Math.min(gameState.pileCount, 6) }).map((_, i) => (
              <div key={i} style={{
                position: 'absolute',
                left: `${i * Math.min(gameState.pileCount > 1 ? 6 : 0, 8)}px`,
                top: `${-i * 1}px`,
                width: '52px', height: '72px',
                background: i === Math.min(gameState.pileCount, 6) - 1
                  ? 'linear-gradient(135deg, #3b0764, #6d28d9)'
                  : `hsl(${260 + i * 10}, 60%, ${18 + i * 3}%)`,
                borderRadius: '8px',
                border: `1.5px solid rgba(167,139,250,${0.15 + i * 0.1})`,
                boxShadow: i === Math.min(gameState.pileCount, 6) - 1
                  ? '0 4px 16px rgba(124,58,237,0.35)'
                  : '0 2px 4px rgba(0,0,0,0.3)',
                zIndex: i,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: i === Math.min(gameState.pileCount, 6) - 1 ? '20px' : '14px',
              }}>
      ...
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: '22px', fontWeight: 800, color: '#7c3aed', marginBottom: '6px' }}>
      ...
      {gameState.lastClaim ? (
        ...
        <div style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
          {gameState.status === 'playing' ? 'No claims yet — play to start' : ''}
        </div>
      )}
    </div>
  );
...
function WaitingLobby({ gameState, isHost, isSpectator, onStart, onUpdateSetting }) {
    ...
    <div style={{
      ...
      {isHost && !isSpectator && (
        <div style={{ marginBottom: '20px' }}>
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', letterSpacing: '0.08em', marginBottom: '12px', fontWeight: 600 }}>
            ⚙️ GAME SETTINGS
          </div>
          ...
          <p style={{ marginTop: '10px', fontSize: '11px', color: 'var(--text-muted)', opacity: 0.6 }}>
            Note: Lives and turn timer don't apply in Cards Bluff. First to empty their hand wins.
          </p>
        </div>
      )}
      ...
      {!isHost && !isSpectator && settings.maxPlayers && (
        <div style={{
          ...
          Max players: <strong style={{ color: '#fff' }}>{settings.maxPlayers}</strong>
...
function BluffSettingControl({ label, value, min, max, onChange, isAuto, hint }) {
  return (
    <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: '10px', padding: '12px 10px', textAlign: 'center' }}>
      <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginBottom: '8px', letterSpacing: '0.05em' }}>{label}</div>
      {isAuto ? (
        <>
          <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: '14px', color: 'var(--text-muted)' }}>Auto</div>
          {hint && <div style={{ fontSize: '9px', color: 'var(--text-muted)', opacity: 0.6, marginTop: '4px' }}>{hint}</div>}
        </>
      ) : (
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', justifyContent: 'center' }}>
          <button
            onClick={() => onChange(Math.max(min, value - 1))}
            style={{ background: 'rgba(255,255,255,0.08)', border: 'none', borderRadius: '6px', color: '#fff', width: '26px', height: '26px', fontSize: '14px', cursor: 'pointer' }}
          >−</button>
          <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, minWidth: '28px', textAlign: 'center', fontSize: '14px' }}>{value}</span>
          <button
...
function PlayArea({ myPlayer, selectedCards, claimedRank, lastClaim, onToggleCard, onClaimRank, onPlay, onPass }) {
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(124,58,237,0.3)', borderRadius: '16px', padding: '20px', marginBottom: '12px' }}>
        ...
        <>
          <div style={{ marginBottom: '12px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '8px', letterSpacing: '0.06em' }}>
              CLAIM RANK {lastClaim ? `(must be ≥ ${lastClaim.rank})` : '(any rank)'}
            </div>
          ...
          <button onClick={onPlay} disabled={!claimedRank} style={{
            width: '100%', padding: '12px',
            background: claimedRank ? 'linear-gradient(135deg, #7c3aed, #a855f7)' : 'rgba(255,255,255,0.06)',
            border: 'none', borderRadius: '10px', color: '#fff', fontWeight: 800, fontSize: '14px',
            opacity: claimedRank ? 1 : 0.5, cursor: claimedRank ? 'pointer' : 'not-allowed', marginBottom: '8px',
          }}>
      ...
      {lastClaim && (
        <button onClick={onPass} style={{ ...ghostBtn, width: '100%', marginTop: '8px' }}>⏭️ Pass</button>
      )}
    </div>
  );
}
...
function ChallengeArea({ lastClaim, myPlayer, selectedCards, claimedRank, onToggleCard, onClaimRank, onPlay, onChallenge, onPass }) {
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(255,77,109,0.3)', borderRadius: '16px', padding: '20px', marginBottom: '12px' }}>
      <div style={{ fontSize: '13px', fontWeight: 700, color: '#ff6b8a', marginBottom: '4px', textAlign: 'center' }}>🎯 Your turn</div>
      <div style={{
    ...
    <div style={{ marginBottom: '12px' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
        {hand.map(card => {
          ...
          return (
            <button key={card.id} onClick={() => onToggle(card.id)} style={{
              width: '44px', height: '64px',
              background: selected
                ? (isRed ? 'rgba(255,107,107,0.2)' : 'rgba(124,58,237,0.2)')
                : 'rgba(255,255,255,0.06)',
              ...
              boxShadow: selected ? `0 6px 18px ${isRed ? 'rgba(255,107,107,0.35)' : 'rgba(124,58,237,0.35)'}` : 'none',
            }}>
              {/* Top rank */}
              <span style={{ fontSize: '11px', fontWeight: 800, color: isRed ? '#ff6b6b' : '#e0e0f0', lineHeight: 1, alignSelf: 'flex-start', paddingLeft: '2px' }}>
                {card.rank}
              </span>
              {/* Centre suit */}
              ...
              <span style={{ fontSize: '11px', fontWeight: 800, color: isRed ? '#ff6b6b' : '#e0e0f0', lineHeight: 1, alignSelf: 'flex-end', paddingRight: '2px', transform: 'rotate(180deg)' }}>
                {card.rank}
              </span>
            </button>
          );
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        textAlign: 'center',
        animation: data.wasBluff ? 'bluffCaught 0.6s ease 0.2s, bounce-in 0.3s ease' : 'bounce-in 0.3s ease',
        boxShadow: data.wasBluff ? '0 0 40px rgba(255,77,109,0.2)' : '0 0 40px rgba(34,211,160,0.2)',
      }}>
        <div style={{ fontSize: '52px', marginBottom: '12px' }}>
          {data.wasBluff ? '🎯' : '😤'}
        </div>
        <h2 style={{ fontSize: '24px', fontWeight: 800, marginBottom: '8px', color: data.wasBluff ? '#ff4d6d' : '#22d3a0' }}>
          ...
          {data.actualCards.map((card, i) => {
            ...
            return (
              <div key={card.id} style={{
                ...
                <span style={{ fontSize: '11px', fontWeight: 800, color: isRed ? '#ff6b6b' : '#e0e0f0', alignSelf: 'flex-start', paddingLeft: '2px' }}>{card.rank}</span>
                <span style={{ fontSize: '18px' }}>{card.suit}</span>
                <span style={{ fontSize: '11px', fontWeight: 800, color: isRed ? '#ff6b6b' : '#e0e0f0', alignSelf: 'flex-end', paddingRight: '2px', transform: 'rotate(180deg)' }}>{card.rank}</span>
              </div>
            );
          })}
        </div>
        ...
        <p style={{ fontWeight: 700, color: data.wasBluff ? '#ff4d6d' : '#22d3a0', fontSize: '15px' }}>
          {data.loserName} picks up {data.pileCount} card{data.pileCount !== 1 ? 's' : ''}!
        </p>
      </div>
    </div>
  );
...
const HTP_BLUFF = [
  ...
  { title: '▶️ Your Turn', body: 'Select one or more cards and claim they are all the same rank (e.g., "3× Kings"). The claimed rank must be ≥ the last claim. Your cards go face-down into the pile.' },
  { title: '🎯 Challenge', body: 'If you think the previous player lied, press Challenge. Cards are revealed — bluffing means they pick up the pile. Honest play means YOU pick up the pile.' },
  { title: '⏭️ Pass', body: 'If you believe the last claim, pass your turn. If everyone passes, the pile is cleared and a new round starts.' },
  { title: '🏆 Winning', body: 'First player to play their last card wins — even if challenged!' },
  { title: '💡 Strategy', body: "You don't have to bluff! Playing honestly makes challenges risky. Bluffing is most effective when you need to get rid of cards that don't match the current rank." },
];
