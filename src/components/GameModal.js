...
export default function GameModal({ game, onClose, onNavigate }) {
  ...
  const handleCreate = () => {
    ...
    logger.info('Emitting create_room', { game: game.id, playerName, stableId });
    setLoading(true);
    const event = isBluff ? 'bluff_create_room' : 'create_room';
    logger.socket.emit(event, { playerName: playerName.trim(), settings: defaultSettings, playerId: stableId });
    socket.emit(event, { playerName: playerName.trim(), settings: defaultSettings, playerId: stableId });
  };
  ...
  return (
    ...
    >
      <div style={{
        background: 'var(--bg-card)', border: `1px solid ${game.border}`,
        borderRadius: '20px', padding: '32px', width: '100%', maxWidth: '420px',
        animation: 'bounce-in 0.3s ease', position: 'relative',
      }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '13px', marginTop: '4px' }}>
            {mode === null ? 'Enter your name to play'
              : mode === 'create' ? 'Ready to create your room'
        ...
        {mode === 'create' && (
          <>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setMode(null)} style={backBtnStyle}>← Back</button>
              <button
            ...
            <p style={{ textAlign: 'center', marginTop: '12px', fontSize: '11px', color: 'var(--text-muted)', opacity: 0.55 }}>
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: '12px', color, transition: 'all 0.2s', cursor: 'pointer',
      }}
...
const labelStyle = {
  ...
  marginBottom: '6px', fontWeight: 600, letterSpacing: '0.05em',
};
