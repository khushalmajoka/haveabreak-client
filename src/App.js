...
export default function App() {
  // Returns the game object if enabled, null if disabled or not found
  const getGame = (id) => GAMES.find((g) => g.id === id && g.enabled) || null;
  return (
    <BrowserRouter>
      <SocketProvider>
        <Toaster
        ...
        */}
        <ErrorBoundary>
          <Routes>
            ...
            {/* ── Word Bomb ── */}
            <Route
              path="/wordbomb/:roomCode"
              element={<JoinPage game="wordbomb" />}
            />
            <Route
            ...
            {/* ── Cards Bluff ── */}
            <Route
              path="/cardsbluff/:roomCode"
              element={<JoinPage game="cardsbluff" />}
            />
            <Route
